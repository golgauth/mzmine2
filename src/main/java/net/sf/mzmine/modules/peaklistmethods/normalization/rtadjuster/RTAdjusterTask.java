...
class RTAdjusterTask extends AbstractTask {
	...
	public RTAdjusterTask(final MZmineProject project, final ParameterSet parameters) {

	        this.currentProject = project;
	    
		this.originalPeakLists = parameters.getParameter(RTAdjusterParameters.peakLists).getValue().getMatchingPeakLists();
...
//		minHeight = parameters.getParameter(RTAdjusterParameters.minHeight).getValue();
	...
	public void run() {
		...
		if (goodStandards.size() == 0) {
			...
			this.setErrorMessage("No good standard peak was found");
			return;
		}
