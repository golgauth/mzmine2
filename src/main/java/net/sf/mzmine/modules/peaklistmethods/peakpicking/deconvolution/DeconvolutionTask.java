...
public class DeconvolutionTask extends AbstractTask {
    ...
    // Peak resolver.
    final PeakResolver resolverModule;
    ...
    public DeconvolutionTask(final MZmineProject project, final PeakList list,
        ...
        // Peak resolver.
        resolverModule = parameters.getParameter(PEAK_RESOLVER).getValue()
                .getModule();
            ...
            if (originalPeakList.getNumberOfRawDataFiles() > 1) {
                ...
                try {
                    ...
                    if (resolverModule instanceof RbasedPeakResolver) {
                        ...
                        // connection.
                        String[] reqPackages = ((RbasedPeakResolver) resolverModule)
                                .getRequiredRPackages();
                        String[] reqPackagesVersions = ((RbasedPeakResolver) resolverModule)
                                .getRequiredRPackagesVersions();
                        String callerFeatureName = resolverModule.getName();
                        RSessionWrapper rSession = new RSessionWrapper(
                                callerFeatureName, reqPackages,
                                reqPackagesVersions);
                        rSession.open();
                        ((RbasedPeakResolver) resolverModule).setRsession(
                                rSession, this);
                    }
                    ...
                    // Turn off R instance.
                    if (resolverModule instanceof RbasedPeakResolver) {
                        ((RbasedPeakResolver) resolverModule).getRsession(this)
                                .close(false);
                    }
    ...
    private PeakList resolvePeaks(final PeakList peakList)
        ...
        for (int index = 0; !isCanceled() && index < chromatogramCount; index++) {
            ...
            final Feature[] peaks = resolverModule.resolvePeaks(this,
