...
import net.sf.mzmine.modules.peaklistmethods.peakpicking.deconvolution.RbasedPeakResolver;
 ...
 */
public class CentWaveDetector implements RbasedPeakResolver {
    ...
    // R.
    private static Map<Task, RSessionWrapper> RSESSIONS_REG = new HashMap<Task, RSessionWrapper>();
    ...
    public void setRsession(RSessionWrapper rSession, Task task) {
        CentWaveDetector.RSESSIONS_REG.put(task, rSession);
    }

    @Override
    public RSessionWrapper getRsession(Task task) {
        return CentWaveDetector.RSESSIONS_REG.get(task);
    }

    @Override
    public Feature[] resolvePeaks(final Task task, final Feature chromatogram,
        ...
        try {
            peakMatrix = centWave(task, retentionTimes, intensities,
     ...
     */
    private static double[][] centWave(final Task task,
        ...
        RSessionWrapper rSession = CentWaveDetector.RSESSIONS_REG.get(task);
        ...
        // Do peak picking.
        final Object centWave = roi <= 1 ? null : rSession.collectDoubleMatrix(
                "findPeaks.centWave(xRaw, ppm=0, mzdiff=0, verbose=TRUE"
                        + ", peakwidth=c(" + peakWidth.lowerEndpoint()
                        * SECONDS_PER_MINUTE + ", " + peakWidth.upperEndpoint()
