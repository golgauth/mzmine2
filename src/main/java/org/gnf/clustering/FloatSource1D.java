 ...
 * @version 1.0
 */
public class FloatSource1D implements DataSource
{
	/**
	 ...
	 */
	public FloatSource1D(final float [] arFloats, final int nRowCount, final int nColCount)
	{
		if(arFloats == null)
	 ...
	 * @see org.gnf.clustering.DataSource.
	 */
	public FloatSource1D(final int nRowCount, final int nColCount)
	{
	...
	{
		final int nIdx = nRow*m_nColCount + nCol;
		m_arFloats[nIdx] = fValue;
	}

		
	public DataSource toEmptySource(int nRowCount, int nColCount)
	{
		return new FloatSource1D(nRowCount, nColCount);
	}
	...
	public DataSource subSource(int nRowFr, int nRowTo)
	{
		...
		return new FloatSource1D(arFloats, /*arFlags,*/ nRowCount, m_nColCount);
	}
	...
	public DataSource subSource(int [] nRowIndices)
	{
		...
		for(int n = 0; n < nRowIndices.length; ++n)
		{
			 ...
			 System.arraycopy(m_arFloats, nIdxFr, arFloats, n*m_nColCount, nLength);
		}
		return new FloatSource1D(arFloats, nRowCount, m_nColCount);
	}
	...
	public void copy(final int nFrSrc, final DataSource sourceDst, final int nFrDst, final int nLength)
	{
		...
		if(sourceDst instanceof FloatSource1D)
			System.arraycopy(m_arFloats, nIdxSrcFr, ((FloatSource1D)sourceDst).m_arFloats, nIdxDstFr, nLength*m_nColCount);
		else
		{
